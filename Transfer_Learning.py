...
def data_generator(data,batch_size=10):
    ...
    while True:   
        for offset in range(0, num_samples, batch_size):
            ...
            audio_batch = audio[offset*30:(offset+batch_size)*30]
            ...
            for i in range(batch_size):
              ...
              y_train[i,:] =  y_train[i,:]
            X_train = np.reshape(X_train, (batch_size*30, 25*2048))
            X1 = X_train[:, :-1]
            ...
            X_train = np.concatenate((X1,mix_data), axis=1)
...
if __name__ == '__main__':
    ...
    history = model.fit_generator(train_gen,steps_per_epoch = int(len(train_l)/ batch_size),
        epochs=50,validation_data=val_gen,validation_steps =
